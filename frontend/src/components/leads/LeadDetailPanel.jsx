  ...
  return (
    ...
    <div className="modal-overlay items-end sm:items-center" onClick={onClose}>
      <div className="bg-white dark:bg-surface-900 border border-surface-200 dark:border-surface-800 rounded-t-2xl rounded-b-none sm:rounded-2xl shadow-xl w-full max-w-2xl h-[95vh] sm:h-auto sm:max-h-[90vh] overflow-hidden animate-slide-in sm:mx-4 will-change-transform" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between p-4 sm:p-5 border-b border-surface-200 dark:border-surface-700 gap-3">
          <div className="min-w-0 flex-1">
            <h2 className="text-base sm:text-lg font-bold text-surface-800 dark:text-surface-100 truncate">{lead.full_name}</h2>
            <div className="flex flex-wrap items-center gap-1.5 mt-1.5">
              <span className="font-mono text-[10px] sm:text-xs text-primary-600 dark:text-primary-400 bg-primary-100 dark:bg-primary-500/10 px-1.5 py-0.5 rounded flex-shrink-0">{lead.lead_code}</span>
              <span className={`badge text-[10px] sm:text-xs px-1.5 py-0.5 ${levelInfo.bgClass} flex-shrink-0`}>{lead.level_code}</span>
              <span className="text-[10px] sm:text-xs text-surface-500 truncate">{levelInfo.label}</span>
            </div>
          </div>
          <div className="flex items-center gap-1.5 flex-shrink-0">
            {activeTab === 'info' && (
              !editing ? (
                <button onClick={() => setEditing(true)} className="btn-secondary text-xs sm:text-sm px-2.5 sm:px-3 py-1.5">Chỉnh sửa</button>
              ) : (
                <div className="flex gap-1.5">
                  <button onClick={handleSave} disabled={saving} className="btn-primary text-xs sm:text-sm px-2.5 sm:px-3 py-1.5">
                    {saving ? 'Lưu...' : 'Lưu'}
                  </button>
                  <button onClick={() => { setEditing(false); setLevelNote(''); }} className="btn-ghost text-xs sm:text-sm px-2 sm:px-3 py-1.5">Hủy</button>
                </div>
              )
            )}
          ...
          {activeTab === 'info' && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3 sm:gap-4">
                  ...
                  <div className="col-span-2 border-t border-surface-200 dark:border-surface-700/50 pt-4 mt-2">
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:gap-4">
                      {customFieldsDef.map((field) => (
                        <div key={field.key} className={field.type === 'text' && field.key.includes('dia_chi') ? 'col-span-2' : 'col-span-1'}>
                ...
                <div className="col-span-2 grid grid-cols-3 gap-3 sm:gap-4 border-t border-surface-200 dark:border-surface-700/50 pt-4 mt-2">
                ...
                <div className="col-span-2 grid grid-cols-2 gap-3 sm:gap-4 border-t border-surface-200 dark:border-surface-700/50 pt-4 mt-2">

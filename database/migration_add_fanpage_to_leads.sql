-- ============================================================
-- Migration: Bổ sung trường Fanpage cho Leads & Sync RPCs
-- Chạy file này trên Supabase SQL Editor
-- ============================================================

-- 1. Thêm cột fanpage vào bảng leads
ALTER TABLE leads ADD COLUMN IF NOT EXISTS fanpage VARCHAR(255) DEFAULT NULL;

-- 2. Dọn dẹp và cập nhật hàm rpc_sync_inbound
DO $$
DECLARE
    r RECORD;
BEGIN
...
BEGIN
  ...
  -- Kiểm tra row_hash trùng với level_code mặc định là L1.KK (chống ghi trùng)
  ...
  INSERT INTO leads (full_name, phone, child_birth_year, child_name, address, source_type, ad_campaign, interested_products, external_source, sheet_in_row, custom_fields, fanpage)
  VALUES (p_full_name, NULLIF(p_phone, ''), p_child_birth_year, p_child_name, p_address, COALESCE(p_source_type, 'PULL'), p_ad_campaign, p_interested_products, 'sheet_in', p_sheet_row, p_custom_fields, p_fanpage)
...
-- 3. Cập nhật hàm rpc_update_lead
...
BEGIN
  ...
  UPDATE leads SET
    ...
    fanpage             = CASE WHEN p_changes ? 'fanpage' THEN (p_changes->>'fanpage')::varchar ELSE fanpage END
  WHERE id = p_lead_id;
...
-- 4. Cập nhật hàm rpc_get_leads_for_outbound_sync
...
-- 5. Nạp lại schema cache của PostgREST

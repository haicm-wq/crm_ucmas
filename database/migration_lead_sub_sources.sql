-- ============================================================
-- Migration: Quản lý nguồn lead con & Cập nhật lọc báo cáo Dashboard
-- ============================================================

-- 1. Tạo bảng nguồn lead con
CREATE TABLE IF NOT EXISTS lead_sub_sources (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    source_type VARCHAR(10) NOT NULL CHECK (source_type IN ('PULL', 'PUSH')),
...
-- 2. Cài đặt bảo mật Row Level Security (RLS)
ALTER TABLE lead_sub_sources ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "sub_sources_select" ON lead_sub_sources;
CREATE POLICY "sub_sources_select" ON lead_sub_sources FOR SELECT
  USING (true);

DROP POLICY IF EXISTS "sub_sources_all_admin" ON lead_sub_sources;
CREATE POLICY "sub_sources_all_admin" ON lead_sub_sources FOR ALL
  USING (auth_permission_group() = 'admin');

GRANT ALL ON lead_sub_sources TO postgres, service_role, anon, authenticated;

-- 3. Seed dữ liệu mặc định ban đầu
INSERT INTO lead_sub_sources (source_type, name) VALUES
    ('PULL', 'Facebook Ads'),
    ('PULL', 'Google Ads'),
    ('PULL', 'Tiktok Ads'),
    ('PULL', 'Website'),
    ('PULL', 'Fanpage'),
    ('PULL', 'Hotline'),
    ('PUSH', 'Giới thiệu (Mất phí)'),
    ('PUSH', 'Giới thiệu (Miễn phí)'),
    ('PUSH', 'Nội bộ'),
    ('PUSH', 'Đại sứ'),
    ('PUSH', 'Trường học'),
    ('PUSH', 'Sự kiện')
...
-- 4. Cập nhật hàm rpc_dashboard_analytics hỗ trợ lọc theo nguồn con (p_sub_source)
DROP FUNCTION IF EXISTS rpc_dashboard_analytics(TIMESTAMPTZ, TIMESTAMPTZ, UUID[], TEXT[], TEXT);
DROP FUNCTION IF EXISTS rpc_dashboard_analytics(TIMESTAMPTZ, TIMESTAMPTZ, UUID[], TEXT[], TEXT, TEXT);
...
CREATE OR REPLACE FUNCTION rpc_dashboard_analytics(
  ...
  p_sub_source TEXT DEFAULT NULL
)
RETURNS JSONB AS $$
DECLARE
  v_from TIMESTAMPTZ;
...
BEGIN
  ...
  WITH filtered_leads AS (
    ...
    WHERE
      ...
      AND (p_sub_source IS NULL OR l.ad_campaign = p_sub_source)
  ),

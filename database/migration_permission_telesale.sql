-- ============================================================
-- MIGRATION: Cập nhật phân quyền Center & Bổ sung nhóm quyền "Sale đặt lịch" (telesale)
-- ============================================================

-- 1. Cập nhật CHECK constraint trên bảng profiles và sub_departments để cho phép quyền 'telesale'
ALTER TABLE profiles DROP CONSTRAINT IF EXISTS profiles_permission_group_check;
ALTER TABLE profiles ADD CONSTRAINT profiles_permission_group_check CHECK (permission_group IN ('admin','marketing','center','telesale'));

ALTER TABLE sub_departments DROP CONSTRAINT IF EXISTS sub_departments_default_permission_group_check;
ALTER TABLE sub_departments ADD CONSTRAINT sub_departments_default_permission_group_check CHECK (default_permission_group IN ('admin','marketing','center','telesale'));


-- 2. Cập nhật RLS cho bảng leads
-- Giữ nguyên hạn chế L0 đối với Center (Center chỉ xem được L1-L4 của trung tâm mình)
...
-- Thêm chính sách RLS cho quyền Telesale (Sale đặt lịch)
-- Xem và sửa lead gán cho mình hoặc đang ở kho L0
...
-- 3. Cập nhật RLS cho bảng appointment_reminders và appointment_comments
...
CREATE POLICY "reminders_insert" ON appointment_reminders FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM profiles p WHERE p.id = auth.uid() AND p.permission_group IN ('admin','marketing','center','telesale'))
);
...
-- 4. Cập nhật hàm rpc_update_lead
...
-- 5. Cập nhật hàm rpc_update_lead_product_level
...
BEGIN
  ...
  IF auth_permission_group() NOT IN ('admin', 'marketing', 'telesale') THEN
  ...
  IF auth_permission_group() = 'telesale' THEN
    v_staff_id := auth.uid();
  ELSE
    v_staff_id := p_staff_id; -- Admin/Marketing có thể gán cho staff cụ thể hoặc để trống
  END IF;
  ...
  WITH updated AS (
    ...
    WHERE id = ANY(p_lead_ids)
      AND (level_group = 'L0' OR auth_permission_group() IN ('admin', 'marketing')) -- Telesale chỉ tự nhận lead L0

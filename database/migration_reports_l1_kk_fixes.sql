...
DROP FUNCTION IF EXISTS rpc_report_booking_sale_performance(TIMESTAMPTZ, TIMESTAMPTZ, UUID);
...
BEGIN
    ...
    -- Computed fields
    NEW.level_group := CASE WHEN NEW.level_code = 'L1.KK' THEN 'L0' ELSE 'L' || COALESCE(substring(NEW.level_code FROM '^L(\d)'), '0') END;
...
-- 4.5. Cập nhật hàm rpc_report_booking_sale_performance
  ...
  FROM (
    ...
    leads_flags AS (
      SELECT 
        ...
        -- L0 (L1.KK): Vào kho L1 kho kiểm trong kỳ
...
GRANT EXECUTE ON FUNCTION rpc_report_booking_sale_performance(TIMESTAMPTZ, TIMESTAMPTZ, UUID) TO anon, authenticated;
...
-- 6. Dọn dẹp cấu hình Levels sản phẩm: Loại bỏ L0 cũ và xếp L1.KK lên đầu
DELETE FROM public.product_levels WHERE level_code = 'L0';
UPDATE public.product_levels SET sort_order = 1 WHERE level_code = 'L1.KK';

-- Đặt cột mặc định và chuẩn hóa dữ liệu L0 cũ sang L1.KK

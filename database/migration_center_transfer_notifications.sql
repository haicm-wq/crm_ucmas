-- ============================================================
-- MIGRATION: Bổ sung thông báo khi chuyển trung tâm
...
BEGIN
  ...
  -- Chuyển trung tâm (thông báo cho cả 2 trung tâm bị chuyển và được chuyển)
  IF v_old_center IS DISTINCT FROM v_new_center AND v_old_center IS NOT NULL AND v_new_center IS NOT NULL THEN
    -- Thông báo cho trung tâm cũ (bị chuyển)
    INSERT INTO notifications (user_id, type, lead_id, message)
    SELECT p.id, 'center_transfer', p_lead_id,
      '➡️ Chuyển đi: Lead ' || v_lead.full_name || ' (' || v_lead.lead_code || ') đã được chuyển từ trung tâm ' || 
      (SELECT name FROM centers WHERE id = v_old_center) || ' sang trung tâm ' || 
      (SELECT name FROM centers WHERE id = v_new_center)
    FROM profiles p 
    WHERE p.center_id = v_old_center AND p.is_active = true;

    -- Thông báo cho trung tâm mới (được chuyển)
    INSERT INTO notifications (user_id, type, lead_id, message)
    SELECT p.id, 'center_transfer', p_lead_id,
      '📥 Nhận chuyển: Lead ' || v_lead.full_name || ' (' || v_lead.lead_code || ') được chuyển đến từ trung tâm ' || 
      (SELECT name FROM centers WHERE id = v_old_center)
    FROM profiles p 
    WHERE p.center_id = v_new_center AND p.is_active = true;
  END IF;
